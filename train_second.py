...
class TimeElapsed():
    ...
    def __call__(self, tag):
        self.now = time.time()
        time_elapsed = self.now - self.start
        self.start = self.now
        print("tag {}, time elapsed: {:.4f}s".format(tag, time_elapsed))
...
def main(config_path):
    ...
    print("Device IDs used: ", model['decoder'].device_ids)
    ...
    for epoch in range(start_epoch, epochs):
        ...
        for i, batch in enumerate(tqdm(train_dataloader)):
            ...
            with torch.no_grad():
                ...
                if multispeaker and epoch >= diff_epoch:
                    ...
                    ref_acoustic_style = model.acoustic_style_encoder(ref_mels.unsqueeze(1))
            ...
            if epoch >= diff_epoch:
                ...
                if multispeaker:
                    ...
                    loss_diff = model.diffusion(s_trg.unsqueeze(1), embedding=bert_dur, features=ref_style).mean() # EDM loss
            ...
            # st = []
            # p_en = []
            acoustic_text_gt_upsampleds = []
            gt_mels = []
            style_ref_mels = []
            prosodic_text_gt_upsampleds = []
            gt_wav = []
            for bib in range(len(mel_input_length)):
                ...
                acoustic_text_gt_upsampleds.append(acoustic_text_gt_upsampled[bib, :, random_start:random_start+mel_len])
                prosodic_text_gt_upsampleds.append(prosodic_text_gt_upsampled[bib, :, random_start:random_start+mel_len])
            ...
            acoustic_text_gt_upsampleds = torch.stack(acoustic_text_gt_upsampleds)
            gt_mels = torch.stack(gt_mels).detach()
            prosodic_text_gt_upsampleds = torch.stack(prosodic_text_gt_upsampleds)
            style_ref_mels = torch.stack(style_ref_mels).detach()
            
            if gt_mels.size(-1) < 80:
                continue
            ...
            with torch.no_grad():
                ...
                N_real = log_norm(gt_mels.unsqueeze(1)).squeeze(1)
                
                y_rec_gt = gt_wav.unsqueeze(1)
                y_rec_gt_pred = model.decoder(acoustic_text_gt_upsampleds, F0_real, N_real, acoustic_style) # en : [B, C, T//2], F0_real : [B, T], N_real : [B, T], s : [B, D]

                if epoch >= joint_epoch:
                    # ground truth = gt recording
                    gt_wav = y_rec_gt # use gt recording since decoder is tuned
                else:
            ...
            loss_mel = stft_loss(y_rec, gt_wav)
            if start_ds:
                loss_gen_all = gl(gt_wav, y_rec).mean()
            else:
                loss_gen_all = 0
            ...
            for batch_idx, batch in enumerate(tqdm(val_dataloader)):
                ...
                try:
                    ...
                    for bib in range(len(mel_input_length)):
                        ...
                        gt_wav.append(torch.from_numpy(y).to(device))
                    ...
                    prosodic_style = model.prosodic_style_encoder(gt_mels.unsqueeze(1))
                    ...
                    for _s2s_pred, _text_input, _text_length in zip(pred_dur, (gt_dur), input_lengths):
                        ...
                        col_range = torch.arange(_s2s_trg.shape[1], device=_s2s_trg.device).unsqueeze(0)  # shape: [1, max_dur]
                        _s2s_trg = (col_range < _text_input.unsqueeze(1)).float()  # shape: [L, max_dur] -> for each text frame, fill up 1s for number of duration
                    ...
                    acoustic_style = model.acoustic_style_encoder(gt_mels.unsqueeze(1))

                    # y_rec = model.decoder(en, F0_fake, N_fake, acoustic_style)
                    y_rec = model.decoder(acoustic_text_gt_upsampleds, F0_fake, N_fake, acoustic_style)
                    loss_mel = stft_loss(y_rec.squeeze(), gt_wav.detach())
        ...
        if epoch < joint_epoch:
            ...
            with torch.no_grad():
                for bib in range(len(acoustic_text_gt_upsampled)):
                    mel_length = int(mel_input_length[bib].item())
                    gt_mels = mels[bib, :, :mel_length].unsqueeze(0)
                    acoustic_text_gt_upsampled_ = acoustic_text_gt_upsampled[bib, :, :mel_length // 2].unsqueeze(0)

                    F0_real, _, _ = model.pitch_extractor(gt_mels.unsqueeze(1))
                    acoustic_style = model.acoustic_style_encoder(gt_mels.unsqueeze(1))
                    real_norm = log_norm(gt_mels.unsqueeze(1)).squeeze(1)

                    y_rec = model.decoder(acoustic_text_gt_upsampled_, F0_real, real_norm, acoustic_style)

                    writer.add_audio('eval/y' + str(bib), y_rec.cpu().numpy().squeeze(), epoch, sample_rate=sr)

                    prosodic_style= model.prosodic_style_encoder(gt_mels.unsqueeze(1))
                    # p_en = p[bib, :, :mel_length // 2].unsqueeze(0)
                    prosodic_text_gt_upsampled_ = prosodic_text_gt_upsampled[bib, :, :mel_length // 2].unsqueeze(0)

                    F0_fake, N_fake = model.duration_prosody_predictor.F0Ntrain(prosodic_text_gt_upsampled_, prosodic_style)

                    y_pred = model.decoder(acoustic_text_gt_upsampled_, F0_fake, N_fake, acoustic_style)
            ...
            with torch.no_grad():
                ...
                for bib in range(len(pred_bert)):
                    if multispeaker:
                        s_pred = sampler(noise = torch.randn((1, 256)).unsqueeze(1).to(texts.device), 
                              embedding=bert_dur[bib].unsqueeze(0),
                              embedding_scale=1,
                                features=ref_style[bib].unsqueeze(0), # reference from the same speaker as the embedding
                    ...
                    d = model.duration_prosody_predictor.text_encoder(pred_bert[bib, :, :input_lengths[bib]].unsqueeze(0), 
                                                     prosodic_style, input_lengths[bib, ...].unsqueeze(0), text_mask[bib, :input_lengths[bib]].unsqueeze(0))
                    ...
                    acoustic_text_gt_upsampleds = (d.transpose(-1, -2) @ pred_aln_trg.unsqueeze(0).to(texts.device))
                    F0_pred, N_pred = model.duration_prosody_predictor.F0Ntrain(acoustic_text_gt_upsampleds, prosodic_style)
                    out = model.decoder((t_en[bib, :, :input_lengths[bib]].unsqueeze(0) @ pred_aln_trg.unsqueeze(0).to(texts.device)), 
                                            F0_pred, N_pred, acoustic_style.squeeze().unsqueeze(0))
