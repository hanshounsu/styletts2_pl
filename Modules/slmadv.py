...
class SLMAdversarialLoss(torch.nn.Module):
    ...
    def forward(self, iters, y_rec_gt, y_rec_gt_pred, waves, mel_input_length, ref_text, ref_lengths, use_ind, s_trg, ref_s=None):
        ...
        pred_bert = self.model.bert_encoder(bert_dur).transpose(-1, -2)
        # Seems like joint training (250302 hounsu)
        ...
        # s_dur = s_preds[:, 128:]
        # s = s_preds[:, :128]
        prosodic_style = s_preds[:, 128:]
        acoustic_style = s_preds[:, :128]

        # d, _ = self.model.duration_prosody_predictor(pred_bert, prosodic_style,
        pred_dur, _ = self.model.duration_prosody_predictor(pred_bert, prosodic_style,
                                                            ref_lengths,
        ...
        for _s2s_pred, _text_length in zip(pred_dur, ref_lengths):
            ...
            t = torch.arange(0, l).unsqueeze(0).expand(
                (len(_s2s_pred), l)).to(ref_text.device)
            loc = torch.cumsum(_dur_pred, dim=0) - _dur_pred / 2

            h = torch.exp(-0.5 * torch.square(t -
                          (l - loc.unsqueeze(-1))) / (self.sig)**2)
        ...
        s2s_attn = torch.zeros(len(ref_lengths), int(
            ref_lengths.max()), max_len).to(ref_text.device)
            ...
            if use_rec:
                if wav.size(-1) > y_pred.size(-1):
                    ...
                    out_org = self.wl.discriminator_forward(
                        wav.detach().squeeze())
                    loss_reg = F.l1_loss(
                        out_crop, out_org[..., :out_crop.size(-1)])

                    if np.random.randint(0, 2) == 0:
                        d_loss = self.wl.discriminator(
                            real_GP.detach().squeeze(), y_pred.detach().squeeze()).mean()
                    else:
                        d_loss = self.wl.discriminator(
                            wav.detach().squeeze(), y_pred.detach().squeeze()).mean()
                else:
                    ...
                    if np.random.randint(0, 2) == 0:
                        d_loss = self.wl.discriminator(
                            wav.detach().squeeze(), real_GP.detach().squeeze()).mean()
                    else:
                        d_loss = self.wl.discriminator(
                            wav.detach().squeeze(), y_pred.detach().squeeze()).mean()
